...
const moodLabels: Record<number, string> = { 5: "Great", 4: "Good", 3: "Okay", 2: "Low", 1: "Difficult" };
...
export default async function MyJournalPage() {
  ...
  const { data: entries } = await supabase
    ...
    .select("id,mood,title,body,tags,created_at")
    .eq("user_id", user.id)
    ...
    <section className="memberDashboard journalWorkspace">
      ...
      <div className="historyList">
        ...
        {(entries || []).map(entry => <article className="dashboardCard journalEntry" key={entry.id}>
          <p className="eyebrow">{new Date(entry.created_at).toLocaleDateString()} {entry.mood ? `· ${moodLabels[entry.mood] || "Check-in"}` : ""}</p>
