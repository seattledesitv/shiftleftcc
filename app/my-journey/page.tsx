...
export default async function MyJourneyPage() {
  ...
  const [{ data: latestResult }, { data: activeSession }, { count: completedCount }, { count: activityCount }, { count: goalCount }, { data: mission }, { data: vision }] = await Promise.all([
  ...
  return <main>
    <section className="pageHero compactHero dashboardHero"><p className="eyebrow">MY JOURNEY</p><h1>Welcome back.</h1><p className="lead">Your private space for mission, vision, life goals, assessments, activity, progress, and reflection.</p></section>
    <nav className="journeyNav" aria-label="My Journey navigation"><Link href="/my-journey">Home</Link><Link href="/my-journey/foundation">Mission & Vision</Link><Link href="/my-journey/goals">Life Goals</Link><Link href="/my-journey/assessments">Assessments</Link><Link href="/my-journey/progress">Progress</Link><Link href="/my-journey/journal">Journal</Link><Link href="/my-journey/activity">Activity</Link><Link href="/resources">Resources</Link><Link href="/book">Coaching</Link></nav>
    <section className="memberDashboard"><div className="dashboardGrid">
      ...
      <article className="dashboardCard"><p className="eyebrow">MY LIFE GOALS</p><h2>{goalCount || 0} active goal{goalCount === 1 ? "" : "s"}</h2><p>Track meaningful annual goals—not only habits or activities.</p><Link className="button secondary" href="/my-journey/goals">Open life goals</Link></article>
