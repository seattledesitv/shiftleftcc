...
export async function POST(request: Request) {
  ...
  if (["checkout.session.async_payment_failed","checkout.session.expired"].includes(event.type)) {
    ...
    if (session.metadata?.commerceOrderId) await admin.from("commerce_orders").update({payment_status:status,updated_at:new Date().toISOString()}).eq("id",session.metadata.commerceOrderId);
