...
export default function WhyShiftLeftPage(){return <main>
  ...
  <section className="applicationSection"><div className="sectionHeading"><p className="eyebrow">WHERE IT APPLIES</p><h2>One strategy. Different human systems.</h2></div><div className="applicationGrid">{applications.map(([title,copy])=><article key={title}><h3>{title}</h3><p>{copy}</p></article>)}</div><div className="actions"><Link className="button secondary" href="/for-who">Explore who it is for</Link><Link className="button secondary" href="/how-it-works">See how it works</Link></div></section>
