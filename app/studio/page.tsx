...
export default async function StudioPage() {
  ...
  const [{ count: members }, { count: results }, { count: sessions }, { count: leads }, { count: bookOrders }, { count: books }, { count: products }, { count: commerceOrders }, { count: seoPages }, { count: platformSettings }, { count: navigationItems }, { count: events }, { count: eventOrders }] = await Promise.all([
  ...
  return <main><section className="pageHero compactHero dashboardHero"><p className="eyebrow">SHIFT LEFT STUDIO</p><h1>Admin workspace.</h1><p className="lead">Manage website configuration, navigation, events, commerce, programs, SEO, assessments, members, leads, books, orders, content, resources, and operations.</p></section><nav className="journeyNav studioNav"><Link href="/studio">Overview</Link><Link href="/studio/platform-settings">Platform</Link><Link href="/studio/navigation">Navigation</Link><Link href="/studio/events">Events</Link><Link href="/studio/commerce">Commerce</Link><Link href="/studio/commerce-orders">Orders</Link><Link href="/studio/seo">SEO Center</Link><Link href="/studio/members">Members</Link><Link href="/studio/assessments">Assessments</Link><Link href="/studio/leads">Leads</Link><Link href="/studio/books">Books</Link><Link href="/studio/book-orders">Book Orders</Link></nav><section className="memberDashboard"><div className="dashboardGrid">
    ...
    <article className="dashboardCard"><p className="eyebrow">EVENTS &amp; TICKETING</p><div className="dashboardScore">{events || 0}</div><h2>Events</h2><p>{eventOrders || 0} registrations/orders across all events.</p><Link className="button secondary" href="/studio/events">Manage events</Link></article>
