...
export default async function StudioPage() {
  ...
  const [{ count: members }, { count: results }, { count: sessions }, { count: leads }, { count: bookOrders }, { count: books }, { count: products }, { count: commerceOrders }] = await Promise.all([
  ...
  return <main><section className="pageHero compactHero dashboardHero"><p className="eyebrow">SHIFT LEFT STUDIO</p><h1>Admin workspace.</h1><p className="lead">Manage commerce, programs, assessments, members, leads, books, orders, content, resources, and platform operations.</p></section><nav className="journeyNav studioNav"><Link href="/studio">Overview</Link><Link href="/studio/commerce">Commerce</Link><Link href="/studio/commerce-orders">Orders</Link><Link href="/studio/members">Members</Link><Link href="/studio/assessments">Assessments</Link><Link href="/studio/leads">Leads</Link><Link href="/studio/books">Books</Link><Link href="/studio/book-orders">Book Orders</Link></nav><section className="memberDashboard"><div className="dashboardGrid">
