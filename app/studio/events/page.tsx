...
export default async function StudioEventsPage() {
  ...
  const { data: events } = await supabase.from("events").select("id,title,slug,image_url,starts_at,status,capacity,event_type").order("starts_at", { ascending: false });

  return <main>
    <section className="pageHero compactHero dashboardHero"><p className="eyebrow">SHIFT LEFT STUDIO · EVENTS</p><h1>Events &amp; ticketing.</h1><p className="lead">Create free or paid events, upload event artwork, configure ticket types, publish registration pages, and track orders per event.</p></section>
    ...
    <section className="memberDashboard">
      ...
      <div className="dashboardGrid" style={{marginTop:24}}>
        {(events || []).map(event => <article className="dashboardCard" key={event.id}>{event.image_url ? <img src={event.image_url} alt={`${event.title} event`} style={{width:"100%",aspectRatio:"16/9",objectFit:"cover",borderRadius:16,marginBottom:14}} /> : null}<p className="eyebrow">{event.status.toUpperCase()}</p><h2>{event.title}</h2><p>{new Date(event.starts_at).toLocaleString()}</p><p>{event.event_type.replace("_"," ")} · Capacity {event.capacity ?? "Unlimited"}</p><div style={{display:"flex",gap:10,flexWrap:"wrap"}}><Link className="button secondary" href={`/studio/events/${event.id}`}>Manage event</Link><Link className="button secondary" href={`/events/${event.slug}`}>Public page</Link></div></article>)}
