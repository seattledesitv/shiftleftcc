...
export default function BookPage() {
  ...
  async function submitRequest(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("success");
      setMessage("Thank you. Your request was sent successfully, and we will follow up shortly.");
      form.reset();
      setChallenge(newChallenge());
    } catch (caught) {
      setStatus("error");
      setMessage(caught instanceof Error ? caught.message : "Unable to send your request.");
      setChallenge(newChallenge());
    }
  }
  ...
  return <main>
    ...
    <section className="contact bookingSection">
      <div>
        ...
        <ul><li>Share the challenge, transition, opportunity, or organizational need in front of you.</li><li>Explore whether coaching, consulting, speaking, or a workshop is the best fit.</li><li>Leave with a recommended next step—without pressure to commit.</li></ul>
        <p><strong>Duration:</strong> 30 minutes</p><p><strong>Cost:</strong> Complimentary</p><p><strong>Format:</strong> Video or phone conversation</p>
      </div>
      ...
      <form action="/api/discovery-call" method="post" onSubmit={submitRequest} autoComplete="on">
        ...
        <label>Organization or event name<input name="organization" autoComplete="organization" placeholder="Optional" /></label>
        ...
        <label className="captchaField">Security check: What is {challenge.left} + {challenge.right}?<input type="number" name="captchaAnswer" inputMode="numeric" min="0" max="20" autoComplete="off" required /></label>
        <label className="honeypotField" aria-hidden="true">Website<input name="website" tabIndex={-1} autoComplete="off" /></label>
        <button className="button primary" type="submit" disabled={status === "sending"}>{status === "sending" ? "Sending…" : "Request my discovery call"}</button>
        {message && <p className={`formStatus ${status}`}>{message}</p>}
        <p className="finePrint">Your request is sent securely from the website. No email application is required.</p>
      </form>
    </section>

    <section className="contentTeaser"><div><p className="eyebrow">EXPLORE FIRST</p><h2>Learn more before booking.</h2><p>Read about the strategy, browse current programs, explore organizational solutions, or understand the engagement process.</p></div><div className="contentLinks"><Link href="/why-shift-left">The Shift Left Strategy →</Link><Link href="/programs">Programs →</Link><Link href="/organizations">Organizations →</Link><Link href="/how-it-works">How it works →</Link></div></section>
  </main>;
}
